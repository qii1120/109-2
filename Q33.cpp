...
class Triangle:public Shape
{
  public:
    ...
    Triangle(Segment *lines, int edge){
      triedge=edge;
      for(int i=0;i<edge;i++){
        setsegment(i,lines[i]);
      }
    ...
    float getArea(){
      float s=(this->getsegment(0).getLength()+this->getsegment(1).getLength()+this->getsegment(2).getLength())/2;
      return sqrt(s*(s-this->getsegment(0).getLength())*(s-this->getsegment(1).getLength())*(s-this->getsegment(2).getLength()));
    }
  private:
    int triedge;
};
